const ATOM = {
    ...
    quarkGain() {
        ...
        if (!hasElement(5)) x = x.mul(y.pow(y.mul(0.01).add(1)).max(1))
        if (hasElement(5)) x = x.mul(y.mul(2).pow(y.mul(0.01).add(1)).max(1))
    ...
    gamma_ray: {
        ...
        effect() {
            ...
            let pow = E(2)
            pow = pow.add(pow.min(player.atom.gamma_ray.mul(0.003),0.333))
    ...
    particles: {
        ...
        effect(i) {
            ...
            if (x.gte(1)) x = x.mul(x.max(1).log(8).plus(1))
