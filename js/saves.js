...
Decimal.prototype.scaleName = function (type, id, rev=false) {
    ...
    if (SCALE_START[type][id] && SCALE_POWER[type][id]) {
        ...
        x = x.scale(s,e,type=='instant'||type=='meta'?1:0,rev)
    }
    return x
}
...
function calc(dt) {
    ...
    player.mv.durability = player.mv.durability.min(tmp.mv.maxDurability)
...
function exporty() {
    ...
    a.download = "IMR: New Path Save - "+new Date().toGMTString()+".txt"
    a.click()
}
...
function enterBeta() {
    createPrompt("To enter beta, type your beta tester id!",'import',loadbeta=>{
       let s = convertStringIntoLink('link')
        if ((loadbeta == xdj) ||(loadbeta == yda) || (loadbeta ==cgx) ||(loadbeta == ing) ||(loadbeta == rnd)) {
