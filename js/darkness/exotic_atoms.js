...
const EXOTIC_ATOM = {
    ...
    req() {
        ...
        if (player.dark.exotic_atom.tier >= 17 && (hasElement(325))) r = E('e20000').pow((E(player.dark.exotic_atom.tier).scaleEvery('fusionTier').sub(17).add(1).div(2).add(1)))
        return r
    },
    tier() {
    ...
    milestones: [
        [
            ...
            [a=>{
                ...
                return x.toNumber()
            },x=>`Reduce Hyper-Glory, Meta-Honor and Meta-Prestige Level scaling power.<br> Hyper-Glory: <b>${formatReduction(x)}</b>, Meta-Honor: <b>${player.dark.exotic_atom.tier >= 16 && hasElement(317)?formatReduction(x*5):formatReduction(x)}</b>, Meta-Prestige Level: <b>${player.dark.exotic_atom.tier >= 16 && hasElement(317)?formatReduction(x*10):formatReduction(x)}</b>. Req: 16th Tier`],
        ],[
            [a=>{
...
if (player.dark.exotic_atom.tier >= 24) x =  Decimal.pow(0.635,overflow(a.add(1).log2(),2,0.5).root(4)).div(player.dark.am?tmp.am_mass_eff.max(1):1)
               else if (player.dark.exotic_atom.tier >= 15) x = Decimal.pow(0.95,overflow(a.add(1).log10(),2,0.5).root(4)).div(player.dark.am?tmp.am_mass_eff.max(1):1)
else x = E(0)
                return x.toNumber()
...
if (player.dark.exotic_atom.tier >= 18) x = a.add(1).log(1.01).pow(500).add(1)
               else if (player.dark.exotic_atom.tier >= 17) x = a.add(1).log(1.01).pow(27).add(1)
else x = E(1)
                return x
