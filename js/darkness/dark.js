const DARK = {
    ...
    rayEffect() {
        ...
        if (a.lt(1e12) && a.gte(1)) x.passive = a.pow(0.25).div(1e3)
        if (a.gte(1e12)) x.passive = a.div(1e12).max(1).log10().add(1).pow(2).div(2e5)
        ...
        if (a.gte(1)) x.passive = x.passive.pow(a.log(10).add(1).pow(0.01).max(1))
    ...
    abEff() {
        ...
        if (a.gte('ee7')) {
            x.ea = a.div('ee7').log10().div(1e6).add(1).root(2).softcap(hasElement(31,1)?3:1.75,0.25,0).add(hasElement(312)?elemEffect(312):0).softcap(10,0.1,0)
        }
