const SUPERNOVA = {
    ...
    starGain() {
        ...
        if (player.supernova.stars.gte(1) && player.inf.theorem.lt(1)) x = x.mul(player.supernova.stars.max(1).log(10).plus(1))
    ...
    req(x=player.supernova.times) {
        ...
        maxlimitGal = E('ee35000').pow(player.galaxy.times.scaleEvery('galaxy',false).pow(3e4).root(hasElement(310)?elemEffect(310):1)).mul('ee28500')
        bulk = E(0)
        bulkGal = E(0)
        if (player.stars.points.div('ee28500').gte(1)) bulkGal = player.stars.points.div('ee28500').max(1).log('ee35000').max(0).root(3e4).pow(hasElement(310)?elemEffect(310):1).scaleEvery('galaxy',true).add(1).floor()
